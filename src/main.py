"""
SP-PPO Experiments: Structure-Preserving RL for Language Models
===============================================================
...
--------------------
- PPO actions are squeezed to shape (N,) so log_prob is computed per action.
- lstsq runs on CPU (the CUDA driver fails on rank-deficient systems).
- env.reset / env.step return .clone() copies, not live views.
- The overhead warmup mirrors the timed loop (one policy call).
- contextlib.nullcontext is used on non-CUDA devices instead of
  torch.amp.autocast("cpu", enabled=False).
- hasattr(torch.backends, "mps") guards MPS detection on old PyTorch builds.
- matplotlib.use("Agg") is set before pyplot is imported.
- _total_grad_norm accumulates on device and syncs once with .item().
- Actions are stacked with torch.stack(act_buf).squeeze(1).
- Structure discovery keeps a single last_train_loss scalar.
- The environment is created once in __main__ and shared by all runners.
- A warmup pass runs before overhead timing (CUDA kernel JIT).
- seeds defaults to None (no mutable default argument).
- torch.cuda.manual_seed_all is called for GPU reproducibility.
- Hot-path .to(DEVICE) calls use non_blocking=True.
- Embedding buffers are pre-allocated on DEVICE.
"""
...
# =============================================================================
# DEVICE SELECTION  (hasattr guard for MPS)
# =============================================================================
...
class GPT2EmbeddingProvider:
    ...
    # nullcontext on non-CUDA devices
    def _amp_ctx(self):
        if DEVICE.type == "cuda":
...
class StructureDiscoveryExperiment:
    ...
    def _fit_algebra(
        ...
        # single scalar, no list accumulation
        last_train_loss = 0.0
        ...
        with torch.no_grad():
            ...
            # lstsq on CPU (the CUDA driver fails on rank-deficient systems)
            N_tr  = V_tr.shape[0]
...
class MultiStepTextAlignmentEnv:
    ...
    Design:
      ...
      - env.step(action, M=None):
          If M is None (baseline PPO): nxt = state_embs lookup.
    ...
    def __init__(
        ...
        reward_cap:   float = 1.0,   # upper clip so rewards are a.s. bounded
        ...
        # -- GPT-2 base embeddings (pre-allocated on DEVICE) -------
        ...
        # geo_r rewards policies whose learned M matches M_env.
        #
    ...
    def reset(self) -> torch.Tensor:
        ...
        self._turn = 0
        # clone so callers never hold a live view into the table
    ...
    def step(
        ...
        M      : optional (k,k) matrix from the policy (exp(proj(theta))).
                 ...
                 computing the geometry reward -- this is the causal path.
        ...
        # clone -- no live view into the table
        ...
        if self.reward_noise > 0.0 and task_r > 0.0:
            # Clip on both sides.  With only max(0,.), additive Gaussian noise
            # would leave the reward unbounded above almost
            # surely, invalidating any deterministic |A_t| <= A_max or ||g|| <= G
            # bound.  Clipping to [0, reward_cap] makes the boundedness
            # assumption hold for every sample.
            task_r = float(np.clip(task_r + self.reward_noise * np.random.randn(),
                                   0.0, self.reward_cap))
...
class PPOConfig:
    ...
    adv_norm:         str   = "episode"   # 'episode' | 'rollout' | 'none'
...
class LieStructuredPPO:
    ...
    def _gather_trajectory(self):
        ...
        # For baseline (no theta): M=None -> env.step uses raw state table.
        ...
        # policy(1D state) -> probs (1,A) -> sample() shape (1,)
    ...
    def _compute_advantages(self, rewards, values, last_val, dones=None):
        """GAE with episode-boundary masking and configurable normalization.

        Two details matter:
          1. Terminal masking: without it the GAE recursion bootstraps across
             episode resets, so credit leaks between independent episodes.
          2. Normalization is per episode, not over the whole `steps_per_iter`
             rollout (512 steps).  The manuscript documents a
             per-episode convention, which is what bounds |A| by (N-1)/sqrt(N)
             with N = horizon.  Over a 512-step rollout that bound is
             511/sqrt(512) ~ 22.6, not 19/sqrt(20) ~ 4.25.

        `cfg.adv_norm` selects the convention:
          'episode' (default) -- standardize within each episode segment;
                                 matches the documented convention.
          'rollout'           -- previous behaviour, kept for reproducing the
                                 earlier runs.
        ...
        for t in reversed(range(T)):
            ...
            # mask the bootstrap and the recursion at episode ends
        ...
        if mode == "rollout":
            ...
            for t in range(T):
                if bool(dones[t]) or t == T - 1:
                    seg = adv[start:t + 1]
                    if seg.numel() > 1:
                        # unbiased std: matches torch's default and the
                        # (N-1)/sqrt(N) extremal bound quoted in the paper
    ...
    # ------------------------------------------------------------------
    # single on-device accumulation, one .item() sync
    # ------------------------------------------------------------------
    @staticmethod
...
# =============================================================================
# OVERHEAD MEASUREMENT  (warmup first; one call per warmup step)
# =============================================================================
...
def measure_overhead(
    ...
    def _time_policy(policy: nn.Module, value_net: nn.Module) -> float:
        ...
        # warmup matches the timed loop structure (one forward per step)
        for _ in range(3):
            probs  = policy(dummy)
...
def _seed_all(seed: int) -> None:
    """Seed CPU, GPU, numpy, and Python random."""
...
def run_multiseed(
    ...
    state_dim: int,
    n_actions: int,
    seeds:     Optional[List[int]] = None,  # None avoids a mutable default
) -> dict:
    """
    ...
    for seed in seeds:
        ...
        # AUC-based gain (paper's primary metric). Threshold-crossing is often
        # None (no seed reaches the convergence threshold), so the crossing-based
        # gain would print nan; report the AUC gain instead.
        b_auc = base_res["auc"]; l_auc = lie_res["auc"]
        auc_gain = (l_auc - b_auc) / b_auc * 100 if b_auc else float("nan")
        cross_gain = (bc - lc) / bc * 100 if (bc and lc) else None
        cross_str = f", cross_gain={cross_gain:.1f}%" if cross_gain is not None else ""
        print(f"  Base: cross={bc} final={bf:.2f} AUC={b_auc:.2f} | "
              f"SP-PPO: cross={lc} final={lf:.2f} AUC={l_auc:.2f} | "
              f"AUC_gain={auc_gain:.1f}%{cross_str}")
    ...
    # AUC gain is the primary, always-defined summary.
    auc_gain_pct = (la.mean() - ba.mean()) / ba.mean() * 100 if ba.mean() else float("nan")
    print(f"\nAUC gain (primary): {auc_gain_pct:.1f}%  "
          f"(SP-PPO {la.mean():.3f} vs Baseline {ba.mean():.3f})")
    # Convergence gain only meaningful if some seeds actually crossed the threshold.
    if len(valid_bc) > 0 and len(valid_lc) > 0:
        avg_gain = ((valid_bc.mean() - valid_lc.mean())
                    / max(valid_bc.mean(), 1e-8) * 100)
        print(f"Average convergence gain (crossed seeds only): {avg_gain:.1f}%")
    else:
        print("Average convergence gain: n/a (no seed reached the threshold)")
...
if __name__ == "__main__":
    ...
    # env created once; all RL runners share it
