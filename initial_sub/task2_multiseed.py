...
Pipeline per seed:
  ...
  4. Evaluate: structured (lam=1.5) vs plain (lam=0) across 8 prompts
...
def run_single_seed(
    ...
    Returns dict with:
      ...
      plain_rewards:      list of per-prompt rewards (plain, lam=0)
    ...
    # Evaluate: plain baseline (same architecture, lam=0, fresh theta=0)
    log.info(f"  [Seed {seed}] Evaluating plain baseline...")
    plain = SIPolicy(base_model, tok, alg, None, cfg)
    plain.lmbda = 0.0
    plain.to(cfg.device)
